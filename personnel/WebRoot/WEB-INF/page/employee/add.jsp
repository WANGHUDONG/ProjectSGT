        ...
        <input type="hidden" name="id" id="id" value="${job.id }" >
          ...
          <div class="layui-form-item" >
              <label for="phone" class="layui-form-label">
                  <span class="x-red">*</span>身户籍号
              </label>
              <div class="layui-input-inline">
                  <input type="text" id="reg_num" name=reg_num required="" lay-verify="required"
                  autocomplete="off" class="layui-input" value="${job.reg_num }">
              </div>
          </div>
          <div class="layui-form-item" >
              <label for="phone" class="layui-form-label">
          ...
          <div class="layui-form-item">
              ...
              <div class="layui-input-inline">
                  <select id="fam_id" name="famSta" class="valid" >
                    <c:forEach items="${requestScope.fam_list}" var="line" varStatus="stat">
                    <option value="${line.fam_id}" <c:if test="${job.famSta == line.fam_id}">selected</c:if>>${line.family_status}
                    </option>
                    <%-- <c:if test="${job.job_id != line.id && line.id == 6}"></c:if> --%>
                    </c:forEach>
                  </select>
              </div>
          </div>
           <div class="layui-form-item">
              ...
              <div class="layui-input-inline">
			<input id="sex" type="radio" name="sex" value="1" title="男" <c:if test="${job.sex == 1 }">checked="checked"</c:if>>
      		<input id="sex1" type="radio" name="sex" value="2" title="女" <c:if test="${job.sex == 2 }">checked="checked"</c:if>>
          ...
          <div class="layui-form-item">
              ...
              <div class="layui-input-inline">
                  <select id="job_id" name="job_id" class="valid" >
                    ...
                    <option value="${line.id}" <c:if test="${job.job_id == line.id || job.job_id != line.id && line.id == 6}">selected</c:if>>${line.name}
                    </option>
                    <%-- <c:if test="${job.job_id != line.id && line.id == 6}"></c:if> --%>
                    </c:forEach>
                  </select>
