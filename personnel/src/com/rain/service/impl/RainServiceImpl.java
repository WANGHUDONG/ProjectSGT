...
public class RainServiceImpl implements RainService{
	 ...
	 */
	@Override
	public List<Employee> get_EmployeeList(Page page) {
		// TODO Auto-generated method stub
		/**
		 * 将部门，职位id中的信息提取出来
		 */
		List<Employee> list = employeedao.get_List(page);
		int size = list.size();
	...
	@Override
	public List<Employee> get_EmployeeLikeList(Page page) {
		// TODO Auto-generated method stub
		/**
		 * 将部门，职位id中的信息提取出来
		 */
//		String content = page.getContent();
		List<Employee> list = employeedao.get_LikeList(page);
		int size = list.size();
		List<Employee> list2 = new ArrayList<>();
		for(int i = 0;i<size;i++){
			Employee data = list.get(i);
...
//		notice.setCreate_date(year+month+day);
	...
	@Override
	public Integer findLikeCount(String content) {
		// TODO 自动生成的方法存根
		Integer count = employeedao.getLikeCount(content);
		return count;
	}
	@Override
	public List<Family> findAllFam() {
		// TODO 自动生成的方法存根
		
		List<Family> allFamily = familyDao.findAllFamily();
